...
sp500["preds"] = next_day_predictions
sp500.head(5)

## 6. Measuring error ##

# The actual values are in to_predict, and the predictions are in next_day_predictions.
mse = sum((to_predict - next_day_predictions)**2)/to_predict.shape[0]
...
regressor = LinearRegression()
predictors_train = train[["value"]]
to_predict_train = train["next_day"]

regressor.fit(predictors_train, to_predict_train)

# Generate a list of predictions with our trained linear regression model
next_day_predictions = regressor.predict(test[["value"]])
test["preds"] = next_day_predictions
