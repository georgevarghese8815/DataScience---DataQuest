...
differences_md = [i - values_median for i in values]
median_difference_sum = sum(differences_md)
...
for item in nba_stats["pts"]:
    diff = item - pts_mean
    sq_diff = diff ** 2
    var = var + sq_diff
point_variance = var/nba_stats["pts"].shape[0]
...
probs = norm.pdf(points1, 0, 2)
plt.plot(points1, probs)
plt.show()
