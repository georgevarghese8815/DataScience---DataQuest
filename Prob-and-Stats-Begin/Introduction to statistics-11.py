...
for i, item in enumerate(gender):
    if item == "male":
        male.append(savings[i])
    else:
        female.append(savings[i])
...
plt.hist(student_scores, bins = 20, cumulative = True)
plt.show()
...
plt.show()
meanp = test_scores_positive.mean()
medianp = numpy.median(test_scores_positive)
plt.hist(test_scores_positive)
plt.axvline(meanp, color = "red")
plt.axvline(medianp, color = "b")
plt.show()
...
plt.axvline(new_titanic_survival["age"].mean(), color = "red")
