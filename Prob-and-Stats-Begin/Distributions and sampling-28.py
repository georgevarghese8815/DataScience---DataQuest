...
print(income.head())
income.sort(columns = "median_income", inplace = True)
lowest_income_county = income["county"].iloc[0]
high_pop = income[income["pop_over_25"] > 500000]
high_pop.sort(columns = "median_income", inplace = True)
lowest_income_high_pop_county = high_pop["county"].iloc[0]
...
mean1 = sum(large_sample)/len(large_sample)
deviations_from_mean = (0.18 - mean1)/numpy.std(large_sample)
over_18_count = len([i for i in large_sample if i >= 0.18])
prop = over_18_count/len(large_sample)
...
random.seed(1)
for i in range(1000):
    temp = select_random_sample(100)
    ratio = temp["median_income_hs"]/temp["median_income_college"]
    avrg = ratio.mean()
    mean_ratios.append(avrg)
