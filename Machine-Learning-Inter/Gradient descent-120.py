...
linearReg.fit(pga.distance[:, np.newaxis], pga.accuracy)
theta1 = linearReg.coef_[0]
temp = pga.distance[:, np.newaxis]
...
J = []
for i in range(100):
    costs = cost(theta0, theta1s[i], pga.distance, pga.accuracy)
    J.append(costs)
plt.scatter(theta1s, J)
plt.show()
